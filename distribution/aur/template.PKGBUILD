...
source=('https://dl.noelware.org/charted/server/x86_64/${pkgver}.tar.gz')
sha1sums=('${package.sha1sum}')
...
package() {
    ...
    for dir in lib config bin; do
       cp -r "${srcdir}/${d}" "${pkgdir}/etc/noelware/charted/server/"
    done
}
