...
class AnalyticsProtocolDaemon(
    ...
    private val log by logging<AnalyticsProtocolDaemon>()
    ...
    fun start() {
        ...
        val serverBuilder = AnalyticsServerBuilder(bindIP, config.port.toInt())
        ...
        for (ext in extensions) serverBuilder.withExtension(ext)

        server.value = serverBuilder.build()
        server.value.start()

        // TODO: Allow setting of bind IP, default 0.0.0.0
        val initReq = Requests.InitRequest("0.0.0.0:${config.port}")
        val request = HttpRequest
            .newBuilder("${config.endpoint}/instances/${server.value.instanceUUID()}/init".toUri())
        ...
        return resp.body().use { stream ->
            ...
            if (r.success) {
                ...
                val final = HttpRequest.newBuilder("${config.endpoint}/instances/${server.value.instanceUUID()}/finalize".toUri())
                    .header("Content-Type", "application/json")
