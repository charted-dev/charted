...
class GrantAdminRoleCommand(private val terminal: Terminal): AccountsAwareCommand(
    "Grants a user with administrator permissions",
    name = "grant-admin",
) {
    ...
    override fun execute() {
        ...
        if (user.admin) {
            terminal.logger.warn("User ${if (user.name != null) "${user.name} (@${user.username})" else user.username} already has admin permissions!")
            exitProcess(0)
        }
        ...
        terminal.logger.info("Granted user ${if (user.name != null) "${user.name} (@${user.username})" else user.username} with administrator privileges")
    }
}
