...
class GrantVerifiedPublisherCommand(private val terminal: Terminal): AccountsAwareCommand(
    ...
    override fun execute() {
        ...
        if (user.verifiedPublisher) {
            terminal.logger.warn("User ${if (user.name != null) "${user.name} (@${user.username})" else user.username} already has Verified Publisher!")
            exitProcess(0)
        }
        ...
        transaction {
            UserTable.update({ UserTable.id eq user.id }) {
                ...
                it[verifiedPublisher] = true
            }
        }

        terminal.logger.info("Granted user ${if (user.name != null) "${user.name} (@${user.username})" else user.username} with Verified Publisher")
    }
}
