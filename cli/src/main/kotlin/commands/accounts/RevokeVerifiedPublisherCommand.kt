...
class RevokeVerifiedPublisherCommand(private val terminal: Terminal): AccountsAwareCommand(
    "Grants a user with the Verified Publisher status",
    name = "revoke-vp",
) {
    ...
    override fun execute() {
        ...
        if (!user.verifiedPublisher) {
            terminal.logger.warn("User ${if (user.name != null) "${user.name} (@${user.username})" else user.username} already doesn't have Verified Publisher!")
            exitProcess(0)
        }
        ...
        transaction {
            UserTable.update({ UserTable.id eq user.id }) {
                ...
                it[verifiedPublisher] = false
            }
        }

        terminal.logger.info("Revoked user ${if (user.name != null) "${user.name} (@${user.username})" else user.username}'s verified publisher status")
    }
}
