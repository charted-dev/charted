...
/// Grants a user verified publisher, as a way to indicate that a user is a verified publisher.
...
#[async_trait]
impl AsyncExecute for VerifiedPublisher {
    async fn execute(&self) -> Result<()> {
        ...
        if !self.force {
            match promptly::prompt::<bool, _>(format!(
                "Are you sure you want to grant @{username} ({}) verified publisher status?",
                user.id
            )) {
                Ok(true) => {}
                Ok(false) => {
        ...
        let verified_publisher = !user.verified_publisher;
        info!(
            "persisting user @{username} ({}) admin state to [{}]",
            user.id, verified_publisher
        );

        match sqlx::query("update users set verified_publisher = $1 where id = $2;")
            .bind(verified_publisher)
            .bind(user.id)
            .execute(&mut conn)
            .await
        {
