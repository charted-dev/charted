...
pub async fn run(command: Cmd) -> Result<()> {
    ...
    if !compose_file.try_exists()? || command.overwrite {
        info!(file = %compose_file.display(), "writing new `docker-compose.yml` in");
        let project = include_str!("../../../docker-compose.yml");

        write_to(&compose_file, command.overwrite, project.to_string())?;
    }

    if let (true, true) = (command.elastic, command.meili) {
    ...
    if command.meili {
        ...
        if !config_toml_file.try_exists()? || command.overwrite {
            write_to(&config_toml_file, command.overwrite, config_toml.to_string())?;
        }
    }
    ...
    if command.elastic {
        ...
        if !elasticsearch_yml_file.exists() || command.overwrite {
            write_to(
                &elasticsearch_yml_file,
                command.overwrite,
                elasticsearch_yml.to_string(),
            )?;
        }

        let jvm_options = include_str!("../../../configs/elasticsearch/jvm.options");
        let jvm_options_file = config.join("jvm.options");
        if !jvm_options_file.exists() || command.overwrite {
            write_to(&jvm_options_file, command.overwrite, jvm_options.to_string())?;
        }
        ...
        if !log4j2_properties_file.exists() || command.overwrite {
            write_to(
                &log4j2_properties_file,
                command.overwrite,
                log4j2_properties.to_string(),
            )?;
        }
    }
