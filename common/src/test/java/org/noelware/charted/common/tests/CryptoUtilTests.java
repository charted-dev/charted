...
public class CryptoUtilTests {
    ...
    public void test_md5Hex() {
        final var encoded = CryptoUtils.md5Hex("Hello, world!");
    ...
    public void test_sha256Hex() {
        final var encoded = CryptoUtils.sha256Hex("beep boop ur gay!");
    ...
    @Test
    public void test_checksumHex() {
        final var bytes = "Hello, world?".getBytes();
        try (final var is = new ByteArrayInputStream(bytes)) {
            final var checksum = CryptoUtils.checksumHex(CryptoUtils.ALGORITHM_SHA256, is);
