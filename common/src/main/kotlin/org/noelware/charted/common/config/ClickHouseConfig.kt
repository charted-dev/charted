 ...
 * @param dataReplication If we should enable replication for data. This ensures you have ZooKeeper alongside with
 * ClickHouse with the `{layer}`, `{shard}`, and `{replica}` macros in place. Check in [the documentation](https://charts.noelware.org/self-hosting/data#replication).
 *
...
data class ClickHouseConfig(
    ...
    @SerialName("data_replication")
    val dataReplication: Boolean = false,
