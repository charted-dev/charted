...
/// Runs any internal commands in the `charted-internals` package.
#[derive(clap::Parser)]
pub struct Args {
    #[clap(flatten)]
    common: CommonArgs,
...
pub fn run(Args { common }: Args) -> eyre::Result<()> {
    crate::cargo(common.cargo.as_ref(), "run", |cmd| {
        cmd.args(["--locked", "--package", "charted-internals", "--release"]);
        ...
        cmd.env("RUSTFLAGS", rustflags).env("RUST_BACKTRACE", "1");
