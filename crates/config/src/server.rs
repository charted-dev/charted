...
impl TryFromEnv for Config {
    ...
    fn try_from_env() -> Result<Self::Output, Self::Error> {
        Ok(Config {
            host: match env!("CHARTED_SERVER_HOST") {
                Ok(val) => val,
                ...
                Err(VarError::NotUnicode(_)) => {
                    return Err(eyre!(
                        "failed to represent `CHARTED_SERVER_HOST` environment variable as valid unicode"
                    ))
                }
            },
            ...
            port: match env!("CHARTED_SERVER_PORT") {
                ...
                Err(VarError::NotUnicode(_)) => {
                    return Err(eyre!(
                        "failed to represent `CHARTED_SERVER_PORT` environment variable as valid unicode"
                    ))
                }
            },
