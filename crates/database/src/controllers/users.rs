...
// mod tests {
//     use super::*;
//     use crate::controllers::DbController;
//     use charted_config::caching::Config;
//     use charted_entities::Name;
//     use sqlx::types::chrono::Local;
//     use std::str::FromStr;

//     async fn create_test_user(controller: &super::DbController) -> eyre::Result<User> {
//         let skeleton = User {
//             verified_publisher: true,
//             created_at: Local::now(),
//             updated_at: Local::now(),
//             username: "noel".parse().unwrap(),
//             email: "cutie@floofy.dev".into(),
//             admin: true,
//             id: 1,
...
//         // If we're not on Linux, then Docker tests are ignored.
//         if !cfg!(target_os = "linux") {
//             return Ok(());
//         }

//         // If Docker isn't installed, then just pass the test since we can't run it.
//         let Ok(client) = bollard::Docker::connect_with_defaults() else {
//             return Ok(());
//         };

//         // ensure that we can ping the daemon, if not, then blindly pass
//         if client.ping().await.is_err() {
//             return Ok(());
//         }

//         let _guard = tracing_subscriber::registry()
//             .with(tracing_subscriber::fmt::layer().with_filter(EnvFilter::from_env("INTEGTEST_LOG")))
//             .set_default();

//         let (_, pool) = crate::get_pool().await?;
//         let controller = super::DbController::new(
//             pool.clone(),
//             charted_cache_inmemory::CacheWorker::new(&Config {
//                 max_object_size: (15 * 1024 * 1024).into(),
//                 strategy: charted_config::caching::Strategy::InMemory,
//                 ttl: None,
//             })?,
//         );

//         // Create the test user
//         create_test_user(&controller).await?;

//         controller
//             .get_by(Name::from_str("noel").unwrap())
//             .await
//             .map(|result| {
//                 assert!(result.is_some());
//                 assert_eq!(result.unwrap().username, "noel".parse().unwrap());
//             })
//             .context("failed to get `noel` user by name")
//     }
// }
