...
import org.noelware.charted.lib.avatars.AvatarStorageUtil
...
class UserApiEndpoints(
    ...
    private val avatars: AvatarStorageUtil
...
//        }
    ...
    suspend fun myAvatar(call: ApplicationCall) {
        ...
        val (contentType, bytes) = avatars.get(user)
        call.respond(createOutgoingContentWithBytes(bytes, contentType))
    }
    ...
    suspend fun getUserAvatarByHash(call: ApplicationCall) {
        ...
        val (contentType, bytes) = avatars.get(user, call.parameters["hash"])
        call.respond(createOutgoingContentWithBytes(bytes, contentType))
    }
    ...
    suspend fun uploadAvatar(call: ApplicationCall) {
        ...
        avatars.update(call.currentUser!!, first)
        call.respond(
            HttpStatusCode.Accepted,
            Response.ok()
        )
    ...
    suspend fun indexYaml(call: ApplicationCall) {
        ...
        val result = storage.trailer.open("./metadata/${user.id}/index.yaml")!!
