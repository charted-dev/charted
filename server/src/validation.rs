...
pub fn validate<R, F: Fn(&R) -> Result<(), ValidationErrors>>(receiver: R, func: F) -> Result<R, ApiResponse> {
    match func(&receiver) {
        Ok(()) => Ok(receiver),
        Err(e) => {
...
pub fn validate_email<'a, R: Into<Cow<'a, str>> + Clone>(receiver: R) -> Result<R, ApiResponse> {
