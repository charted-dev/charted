...
@Disabled("Can't replicate testing Elasticsearch yet!")
class ElasticsearchTests {
    private val container: ElasticsearchContainer = ElasticsearchContainer(DockerImageName.parse("docker.elastic.co/elasticsearch/elasticsearch").withTag("8.4.1"))
        .apply {
            ...
            // let elasticsearch consume ~2GB of ram since tests
            // don't index a lot of data.
            withEnv("ES_JAVA_OPTS", "-Xms1024m -Xmx2048m")
        }

    private val client: ElasticsearchService by lazy {
        DefaultElasticsearchService(config, Json)
    }
